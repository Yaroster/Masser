...
public class Monstering extends Listening {
    public Monstering(Masser main) {
        super(main);
    }

    // Event handler for creature spawning
    @EventHandler(priority = EventPriority.NORMAL)
    public void onSpawn(CreatureSpawnEvent event) {
        if (event.getSpawnReason() == SpawnReason.SPAWNER) {
            // Set metadata to mark the entity as spawned from a spawner
        ...
        if (collision != collisionType.Prison) {
            ...
            event.setCancelled(event.getEntity().getType() == EntityType.CREEPER);
        }
    ...
    // Event handler for entity explosion
    @EventHandler(priority = EventPriority.HIGHEST)
    public void onExplode(EntityExplodeEvent event) {
        // Set the location to integer coordinates to prevent floating point issues
    ...
    // Event handler for entity changing block
    @EventHandler(priority = EventPriority.NORMAL)
    ...
    // Event handler for entity interaction
    @EventHandler(priority = EventPriority.NORMAL)
